...
construct_book() {
    ...
    if display_list | grep --quiet "$STR_DATE_VENUE" ; then
        ...
        # 1. option condition :
            ...
            BOOK_DIR=/media/hdd/data/ITCH/book/"$DATE"/"$STOCK"/
            MESS_DIR=/media/hdd/data/ITCH/messages/"$DATE"/"$STOCK"/
            mkdir -p $BOOK_DIR
            mkdir -p $MESS_DIR
...
# set variables
# SCRIPT_NAME="./BookConstructor.sh"
DATA_DIR="./data/"
EXCHANGE_VENUE="NASDAQ"
BOOK_LEVELS=20

# read each ticker from the tickers file...
while IFS= read -r ticker
do
    #  ...for each day of available data...
    while IFS= read -r day
    do
        # ...run the script with the given parameters
        echo "Processing $ticker for day $day"
        construct_book $DATA_DIR $day $EXCHANGE_VENUE $ticker -n $BOOK_LEVELS
    done < days.txt
done < tickers.txt
